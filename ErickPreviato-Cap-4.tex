...
Este capítulo apresenta a avaliação experimental de modelos de linguagem baseados em \textit{Transformers}, aplicados à tarefa de análise de sentimentos em comentários de alunos dos cursos de graduação, que constituem o objeto de estudo desta pesquisa.

\section{Configuração Experimental e Métricas}

Conforme mencionado no capítulo anterior, o conjunto de dados processado foi dividido na proporção de 80\% para treinamento e 20\% para validação,
...
Em linha com o capítulo anterior, a avaliação dos modelos foi conduzida utilizando quatro métricas principais, escolhidas para oferecer uma visão global do desempenho dos classificadores,
especialmente considerando o possível desbalanceamento das classes. As métricas utilizadas foram: Acurácia, \textit{F1-Score}, Coeficiente de Correlação de Matthews (MCC)
e Kappa de Cohen.
...
A Tabela~\ref{tab:resultados_modelos} e a Figura~\ref{fig:resultados_modelos} apresentam um resumo dos resultados obtidos após o \textit{fine-tuning} para cada modelo nas 
...
\section{Análise Qualitativa com Dados Reais}
