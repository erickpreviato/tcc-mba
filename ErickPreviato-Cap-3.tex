...
A Figura~\ref{img:dataset} representa uma amostra do \textit{dataset} para exemplificar a estrutura dos dados.
% Inserir uma imagem
\graphicspath{ {USPSC-img/} }
\begin{figure}[h]
\caption{Amostra do \textit{dataset}}
\label{img:dataset}
\centering
\includegraphics[scale=0.36]{dataset}
\end{figure}
...
Essa amostra foi exportada para um novo arquivo \texttt{.csv}, contendo as colunas com os comentários (C15, C16 e C17). Este arquivo serviu como base para o pré-processamento descrito na Seção~\ref{pre-processamento}, no qual foram adicionadas as colunas de sentimento (\texttt{Sentimento\_C15}, \texttt{Sentimento\_C16} e \texttt{Sentimento\_C17}) com os valores \textit{negativo}, \textit{neutro} e \textit{positivo}.
...
\textbf{Acurácia (Accuracy)}. Mede a proporção de predições corretas em relação ao total de instâncias. É uma métrica simples, mas pode ser enganosa em casos de desequilíbrio de classes.

$$\text{Acurácia} = \frac{VP + VN}{VP + VN + FP + FN}$$

\textbf{Precisão (Precision)}. Mede a proporção de instâncias classificadas como Positivas que são realmente Positivas. Responde à pergunta: "Das que o modelo classificou como Positivas, quantas estavam corretas?".
$$\text{Precisão} = \frac{VP}{VP + FP}$$

\textbf{Revocação (Recall ou Sensibilidade)}. Mede a proporção de instâncias Positivas que foram corretamente identificadas. Responde à pergunta: "Das que são realmente Positivas, quantas o modelo identificou?".
$$\text{Revocação} = \frac{VP}{VP + FN}$$

\textbf{Métrica F1 (F1-Score)}. É a média harmônica da Precisão e da Revocação, sendo uma métrica que busca um equilíbrio entre ambas. É particularmente útil em casos de desequilíbrio de classes, onde se deseja que o modelo tenha tanto alta precisão quanto alta revocação.
